...
from PyQt5.QtGui import QPainter, QColor, QFont, QTextCursor, QDesktopServices, QKeySequence
...
# Versão corrigida - Mantém as referências originais do PDF
# =============================================================================
    ...
    def run(self):
        ...
        try:
            ...
            for kw in ["references", "bibliography", "referências", "bibliografia"]:
                ...
                if found != -1: 
                    content = full_text[found:]; 
                    break
            
            # Regex robusto para separar referências - VERSÃO CORRIGIDA
            # Agora captura o texto completo de cada referência
            segments = []
            raw_segments = re.split(r"\n(?=\[?\d+\]?[\.\s]|\b[A-Z][a-z]+, [A-Z]\.)", content)
            
            for seg in raw_segments:
                seg_clean = seg.strip().replace('\n', ' ')
                if len(seg_clean) > 15:
            ...
            for i, seg in enumerate(segments):
                ...
                try:
                    # Preservar a referência original
                    original_ref = seg
                    
                    # Buscar DOI se existir para enriquecer os dados
                    doi_m = re.search(r"10\.\d{4,9}/[-._;()/:A-Z0-9]+", seg, re.IGNORECASE)
                    item = None
                    if doi_m:
                        try: 
                            item = cr.works(ids=doi_m.group(0).rstrip('.'))['message']
                        except: 
                            item = None
                    ...
                    if item:
                        # Extrair dados do Crossref mas manter referência original
                        auths = item.get('author', item.get('editor', []))
                        auth_str = "; ".join([self.format_person(a) for a in auths]) or "Unknown Author"
                        year = str(item.get("created", {}).get("date-parts", [[None]])[0][0]) if item.get("created") else "n.d."
                        title = self.clean_text(item.get("title", ["Unknown Title"])[0]) if item.get("title") else "Unknown Title"
                        journal = self.clean_text(item.get("container-title", ["N/A"])[0]) if item.get("container-title") else "N/A"
                        ...
                        abstract_clean = self.clean_abstract(abstract_raw) if abstract_raw else "N/A"
                        ...
                        # HTML Rico do Log - AGORA MOSTRA A REFERÊNCIA ORIGINAL
                        detail_log = (
                            ...
                            f"  </div>"
                            f"  <div style='color:#BBB; margin-bottom: 6px; font-style: italic;'>{original_ref[:200]}{'...' if len(original_ref) > 200 else ''}</div>"
                            f"  <div style='color:{sub_color}; font-size: 11px;'>"
                            f"    <span style='color:#00D2FF;'>[Enriquecido]</span> {auth_str} • {journal} • {year} • DOI: {doi_html}"
                            f"  </div>"
                            f"</div>"
                        )
                        ...
                        if not is_duplicate:
                            history[uid] = title
                            # Agora adiciona a referência original na tabela também
                            enriched_ref = f"{original_ref} [DOI: {res_doi}]" if res_doi else original_ref
                            self.item_completed.emit([auth_str, title, year, link, journal, pub, item_type, abstract_clean, original_ref])
                        else:
                            # Para duplicatas, ainda adiciona com marcação
                            self.item_completed.emit([auth_str, title, year, link, journal, pub, item_type, abstract_clean, f"[DUPLICADA] {original_ref}"])
                        
                        self.log_result.emit(detail_log)
                    else:
                        # Quando não encontra na API, usa a referência original
                        clean_ref = self.clean_text(seg)
                        error_log = (
                            f"<div style='{base_item_style} color:#FFAA00;'>"
                            f"  <div style='margin-bottom: 4px;'>"
                            f"    <span style='font-family:Consolas; font-weight:bold; color:#FFAA00;'>[{idx:03d}/{total:03d}]</span>"
                            f"    <span style='color:#FFAA00;'> [Não encontrado no Crossref]</span>"
                            f"  </div>"
                            f"  <div style='color:#BBB;'>{clean_ref}</div>"
                            f"</div>"
                        )
                        self.log_result.emit(error_log)
                        # Adiciona aos dados com a referência original
                        self.item_completed.emit(["Desconhecido", "Título não identificado", "n.d.", "", "N/A", "N/A", "Unknown", "N/A", clean_ref])
                
                except Exception as e:
                    except_log = (
                        f"<div style='{base_item_style} color:#FF3366;'>"
                        f"  <span style='font-family:Consolas; font-weight:bold;'>[{idx:03d}/{total:03d}]</span> "
                        f"  Erro ao processar: {str(e)}<br>"
                        f"  <span style='color:#BBB;'>{seg[:200]}</span>"
                        f"</div>"
                    )
                    self.log_result.emit(except_log)
                    self.item_completed.emit(["Erro", "Erro no processamento", "n.d.", "", "N/A", "N/A", "Error", "N/A", seg])
...
class BiblioApp(QMainWindow):
    ...
    def initUI(self):
        self.setWindowTitle("BiblioTech Ultimate Integrated V2 - Corrigido"); self.resize(1600, 1000)
        self.setStyleSheet("""
        ...
        # ABA 1: SPLITTER (LOG + PDF)
        self.tab1_widget = QWidget()
        self.tab1_layout = QHBoxLayout(self.tab1_widget); self.tab1_layout.setContentsMargins(0, 0, 0, 0)
        ...
        self.shortcut_find.activated.connect(self.trigger_pdf_search)

        # ABA 2: TABELA - AGORA COM 9 COLUNAS (incluindo referência original)
        self.table = QTableWidget(0, 9)
        self.table.setHorizontalHeaderLabels(["AUTORES", "TÍTULO", "ANO", "LINK DOI", "JORNAL", "PUBLISHER", "TIPO", "ABSTRACT", "REFERÊNCIA ORIGINAL"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        self.table.horizontalHeader().resizeSection(1, 250); 
        self.table.horizontalHeader().resizeSection(7, 300)
        self.table.horizontalHeader().resizeSection(8, 400)  # Coluna da referência original maior
    ...
    def trigger_pdf_search(self):
        """Ativa a função de busca interna do motor Chromium"""
        self.pdf_viewer.setFocus()
        self.pdf_viewer.page().triggerAction(self.pdf_viewer.page().Find)
    ...
    def sync_ui(self):
        ...
        for r, row in enumerate(self.live_data):
            # Agora row tem 9 elementos (o último é a referência original)
            for c, val in enumerate(row):
                if c < 9:  # Garantir que temos 9 colunas
                    item = QTableWidgetItem(str(val))
                    if c == 3 and str(val).startswith("http"):  # Coluna DOI
                        item.setForeground(QColor("#00D2FF")); font = item.font(); font.setUnderline(True); item.setFont(font)
                    if c == 7 and val != "N/A":  # Abstract
                        abs_count += 1
                        item.setToolTip(str(val)[:500] + "...")
                    if c == 8:  # Referência original - tornar visualmente distinta
                        item.setForeground(QColor("#FFAA00"))
                        item.setToolTip("Referência original extraída do PDF")
                    self.table.setItem(r, c, item)
            
            # Processar dados para os gráficos (ignorar duplicatas marcadas)
            if not row[8].startswith("[DUPLICADA]"):
        ...
        self.c_tps.update_data(Counter([row[6] for row in self.live_data if not row[8].startswith("[DUPLICADA]")]))
        ...
        if path and self.live_data:
            with open(path, "w", encoding="utf-8") as f:
                for r in self.live_data:
                    # Usar a referência original como fallback se não tiver dados estruturados
                    if r[1] == "Título não identificado":
                        # Criar uma entrada baseada na referência original
                        k = f"ref{hash(r[8]) % 10000}"
                        f.write(f"@misc{{{k},\n  note={{{r[8]}}},\n}}\n\n")
                    else:
                        k = re.sub(r'\W+', '', r[0].split(',')[0]) + str(r[2]) if r[0] != "Desconhecido" else f"ref{hash(r[8]) % 10000}"
    ...
    def export_excel(self):
        ...
        if path and self.live_data:
            df = pd.DataFrame(self.live_data, columns=["Autores", "Título", "Ano", "DOI", "Jornal", "Publisher", "Tipo", "Abstract", "Referência Original"])
