...
class DeepExtractionWorker(QThread):
    ...
    item_completed = pyqtSignal(list) # Retorna dados + Identificador
    ...
    def __init__(self, pdf_path):
        ...
        # Extrai apenas o nome do arquivo para usar como ID
...
# =============================================================================
# GRÁFICOS (Mantidos iguais, lógica de dados será ajustada na Main)
# =============================================================================
...
class BiblioApp(QMainWindow):
    def __init__(self):
        ...
        self.current_status_text = "Pronto."
        self.spinner_chars = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
        self.spin_idx = 0
        self.initUI()
    ...
    def initUI(self):
        ...
        self.right_layout.addWidget(self.btn_sync_table)
        self.right_layout.addWidget(self.btn_sync_dash)
        self.right_layout.addSpacing(15)
        self.right_layout.addWidget(self.btn_bib)
        self.right_layout.addWidget(self.btn_xlsx)
        ...
        # --- ABA 2: TABELA ---
        # Adicionada coluna "ARQUIVO" no índice 0
        ...
        self.table.horizontalHeader().resizeSection(0, 150) # Coluna Arquivo
        self.table.horizontalHeader().resizeSection(2, 300) # Coluna Título
        ...
        self.tabs.addTab(self.table, "BANCO DE DADOS (ACUMULADO)")
        self.tabs.addTab(dash_scroll, "DASHBOARD ANALÍTICO")

    def setup_dash_widgets(self):
        self.l_dash.setContentsMargins(40, 40, 40, 40); self.l_dash.setSpacing(40)
        
        # Aviso sobre atualização manual
        lbl_info = QLabel("Nota: Para otimizar a performance ao carregar múltiplos arquivos, o Dashboard não atualiza automaticamente. Clique em 'ATUALIZAR DASHBOARD' na barra superior para sincronizar.")
        lbl_info.setStyleSheet("color: #666; background: #111; padding: 10px; border-radius: 6px; font-style: italic;")
        self.l_dash.addWidget(lbl_info)
    ...
    def handle_cell_click(self, row, col):
        if col == 4: # Link DOI (ajustado índice devido à nova coluna)
    ...
    def open_pdf(self):
        ...
        if p:
            ...
            # NÃO limpamos self.live_data, apenas adicionamos mais
            # Limpamos apenas o visualizador de log atual para não ficar confuso, ou mantemos?
            # Vamos inserir uma quebra visual no log.
            ...
            self.worker.item_completed.connect(self.append_data_item) # Nova função de append
    ...
    def update_spinner_animation(self):
        ...
        # Apenas atualiza a barra de status ou título para não poluir o log
        self.setWindowTitle(f"BiblioTech - Processando: {self.spinner_chars[self.spin_idx % len(self.spinner_chars)]} {self.current_status_text}")
        self.spin_idx += 1
    ...
    # --- FUNÇÃO CRÍTICA: ACUMULAÇÃO DE DADOS ---
    def append_data_item(self, item_data):
        # item_data: [File, Auth, Title, Year, Link, Journal, Pub, Type, Abstract]
        self.live_data.append(item_data)
        
        # Opcional: Atualizar tabela em tempo real (pode ser pesado se for muito rápido)
        # Vamos atualizar a contagem apenas
        self.lbl_count.setText(f"{len(self.live_data)} Refs Acumuladas")
        
        # Para feedback visual imediato, adicionamos na tabela agora
        self.add_row_to_table(item_data)
    ...
    def on_process_finished(self, success):
        ...
        self.setWindowTitle("BiblioTech - Multi-Source Accumulator")
        ...
        # NÃO chamamos update_dashboard_view automaticamente para economizar recurso
        # Mas atualizamos a tabela completa para garantir
        self.update_table_view()

    def update_table_view(self):
        """Reconstrói a tabela (caso haja desincronia)"""
        self.table.setRowCount(0)
        self.table.setRowCount(len(self.live_data))
        self.table.setSortingEnabled(False) # Desabilita sort durante inserção para performance
    ...
    def update_dashboard_view(self):
        """Calcula estatísticas pesadas APENAS quando solicitado"""
        if not self.live_data: return
        
        # Índices: 0=File, 1=Auth, 2=Title, 3=Year, 4=Link, 5=Journal, 6=Pub, 7=Type, 8=Abstract
        ...
        self.k4.v.setText(str(len(set(files)))) # Quantos PDFs diferentes
        ...
        if path and self.live_data:
            with open(path, "w", encoding="utf-8") as f:
                for r in self.live_data:
                    # Gera chave única usando autor + ano + índice para evitar colisão
                    k = re.sub(r'\W+', '', r[1].split(',')[0]) + str(r[3])
                    abs_field = f"  abstract={{{r[8]}}},\n" if r[8] != "N/A" else ""
                    # r indices: 0=File, 1=Auth, 2=Title, 3=Year, 4=Link, 5=Journal, 6=Pub
